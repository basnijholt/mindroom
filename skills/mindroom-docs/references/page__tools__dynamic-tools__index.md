...
After `load_tool()` succeeds, the agent should continue the same task and call the loaded tool in a later tool-call step in the same response.
It should not wait for another user message, and it should not call the loaded tool in the same parallel tool-call batch as `load_tool()`.
