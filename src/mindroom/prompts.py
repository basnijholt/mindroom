...
After load_tool succeeds, continue the same task and call the newly loaded tool in a later tool-call step in the same response.
...
DYNAMIC_TOOLS_TOOLKIT_INSTRUCTIONS = (
    ...
    "After load_tool() succeeds, continue the same task and call the loaded tool in a later tool-call step. "
    "Do not wait for another user message or call the loaded tool in the same parallel tool-call batch."
)
