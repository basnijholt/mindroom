...
class TestStreamingBehavior:
    ...
    @pytest.mark.asyncio
    async def test_cancel_sources_set_distinct_terminal_stream_statuses(self) -> None:
        """User stop, generic interruption, and sync restart must not collapse to one stream status."""
        mock_client = _make_matrix_client_mock()
        observed_statuses: dict[str, str] = {}

        cases = (
            ("user_stop", USER_STOP_CANCEL_MSG, STREAM_STATUS_CANCELLED),
            ("interrupted", None, STREAM_STATUS_INTERRUPTED),
        ...
        assert set(observed_statuses.values()) == {
            STREAM_STATUS_CANCELLED,
            STREAM_STATUS_INTERRUPTED,
            STREAM_STATUS_ERROR,
        }
